# Prompt 26: Agent 32 - GRASS GIS Analysis

## Wat deze prompt bouwt

GRASS GIS Analysis voor NOVA v2 pipeline.

## Voorwaarden

- [ ] Prompt 25 compleet
- [ ] agent_25_status.json toont "active" of "fallback_mode"

## De prompt

```
Bouw Agent 32 - GRASS GIS Analysis voor NOVA v2.

CONTEXT:
- Spec: L:\!Nova V2\extensions\nova_v2_extensions\grass\32_grass_gis_analysis_agent.md
- Target: L:\!Nova V2\v2_services\agent_32_grass_gis\
- Hetzner: /docker/nova-v2/services/agent_32_grass_gis/
- Webhook: http://178.104.207.194:5680/webhook/grass-analysis
- Port intern: 8132
...
computational_validity, reasonableness, performance_profiler

ENDPOINTS:
/terrain/slope, /hydrology/watershed, /viewshed

DEPENDENCIES:
grass-session gdal (GRASS on host)
...
10. Status file schrijven naar L:\!Nova V2\status\agent_32_status.json
...
- Agent 32 GRASS GIS Analysis status
- Webhook URL
- Test resultaten  
...
Invoke-RestMethod -Uri "http://178.104.207.194:5680/webhook/grass-analysis" -Method POST -Body $testBody -ContentType "application/json"

# Check status
Get-Content "L:\!Nova V2\status\agent_32_status.json" | ConvertFrom-Json
```
...
- Check container logs: `ssh root@178.104.207.194 "cd /docker/nova-v2 && docker compose logs agent-32-grass_gis"`
- Check dependencies geïnstalleerd als host tools nodig zijn


## Volgende prompt

`04_fase4_advanced/prompt_27_agent_*.md` (zie PROMPT_INDEX.md)
