# Prompt 05: Agent 22 - Blender Game Renderer

## Wat deze prompt bouwt

Blender Game Renderer voor NOVA v2 pipeline.

## Voorwaarden

- [ ] Prompt 04 compleet
- [ ] agent_04_status.json toont "active" of "fallback_mode"

## De prompt

```
Bouw Agent 22 - Blender Game Renderer voor NOVA v2.

CONTEXT:
- Spec: L:\!Nova V2\extensions\nova_v2_extensions\blender_game\22_blender_game_renderer_agent.md
- Target: L:\!Nova V2\v2_services\agent_22_blender_game_renderer\
- Hetzner: /docker/nova-v2/services/agent_22_blender_game_renderer/
- Webhook: http://178.104.207.194:5680/webhook/blender-render
- Port intern: 8122
...
render_consistency, alignment_checker, material_quality

ENDPOINTS:
/render, /render/batch, /render/status

DEPENDENCIES:
opencv-python pillow (Blender 4.x on host)
...
10. Status file schrijven naar L:\!Nova V2\status\agent_22_status.json
...
- Agent 22 Blender Game Renderer status
- Webhook URL
- Test resultaten  
...
Invoke-RestMethod -Uri "http://178.104.207.194:5680/webhook/blender-render" -Method POST -Body $testBody -ContentType "application/json"

# Check status
Get-Content "L:\!Nova V2\status\agent_22_status.json" | ConvertFrom-Json
```
...
- Check container logs: `ssh root@178.104.207.194 "cd /docker/nova-v2 && docker compose logs agent-22-blender_game_renderer"`
- Check dependencies geïnstalleerd als host tools nodig zijn


## Volgende prompt

`01_fase1_foundation/prompt_06_agent_*.md` (zie PROMPT_INDEX.md)
