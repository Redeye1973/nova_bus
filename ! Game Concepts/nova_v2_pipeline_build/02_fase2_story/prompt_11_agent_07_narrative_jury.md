# Prompt 11: Agent 07 - Narrative Jury

## Wat deze prompt bouwt

Narrative Jury voor NOVA v2 pipeline.

## Voorwaarden

- [ ] Prompt 10 compleet
- [ ] agent_10_status.json toont "active" of "fallback_mode"

## De prompt

```
Bouw Agent 07 - Narrative Jury voor NOVA v2.

CONTEXT:
- Spec: L:\!Nova V2\agents\nova_v2_agents\jury_judge\07_narrative_jury.md
- Target: L:\!Nova V2\v2_services\agent_07_narrative_jury\
- Hetzner: /docker/nova-v2/services/agent_07_narrative_jury/
- Webhook: http://178.104.207.194:5680/webhook/narrative-review
- Port intern: 8107
...
canon_adherence, character_voice, world_logic, narrative_arc

ENDPOINTS:
/review/text, /review/canon, /review/voice

DEPENDENCIES:
qdrant-client sentence-transformers (Ollama on host)
...
10. Status file schrijven naar L:\!Nova V2\status\agent_07_status.json
...
- Agent 07 Narrative Jury status
- Webhook URL
- Test resultaten  
...
Invoke-RestMethod -Uri "http://178.104.207.194:5680/webhook/narrative-review" -Method POST -Body $testBody -ContentType "application/json"

# Check status
Get-Content "L:\!Nova V2\status\agent_07_status.json" | ConvertFrom-Json
```
...
- Check container logs: `ssh root@178.104.207.194 "cd /docker/nova-v2 && docker compose logs agent-07-narrative_jury"`
- Check dependencies geïnstalleerd als host tools nodig zijn
- Check Ollama running op PC (tailscale bereikbaar)

## Volgende prompt

`02_fase2_story/prompt_12_agent_*.md` (zie PROMPT_INDEX.md)
