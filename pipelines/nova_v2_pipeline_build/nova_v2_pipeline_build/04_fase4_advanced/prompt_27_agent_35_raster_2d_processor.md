# Prompt 27: Agent 35 - Raster 2D Processor

## Wat deze prompt bouwt

Raster 2D Processor voor NOVA v2 pipeline.

## Voorwaarden

- [ ] Prompt 26 compleet
- [ ] agent_26_status.json toont "active" of "fallback_mode"

## De prompt

```
Bouw Agent 35 - Raster 2D Processor voor NOVA v2.

CONTEXT:
- Spec: L:\!Nova V2\extensions\nova_v2_extensions\raster2d\35_raster_2d_processor_agent.md
- Target: L:\!Nova V2\v2_services\agent_35_raster_2d_processor\
- Hetzner: /docker/nova-v2/services/agent_35_raster_2d_processor/
- Webhook: http://178.104.207.194:5680/webhook/raster-2d
- Port intern: 8135
...
format_compliance, layer_structure, export_quality

ENDPOINTS:
/gimp/process, /krita/process, /inkscape/process

DEPENDENCIES:
psd-tools pillow (GIMP/Krita/Inkscape on host)
...
10. Status file schrijven naar L:\!Nova V2\status\agent_35_status.json
...
- Agent 35 Raster 2D Processor status
- Webhook URL
- Test resultaten  
...
Invoke-RestMethod -Uri "http://178.104.207.194:5680/webhook/raster-2d" -Method POST -Body $testBody -ContentType "application/json"

# Check status
Get-Content "L:\!Nova V2\status\agent_35_status.json" | ConvertFrom-Json
```
...
- Check container logs: `ssh root@178.104.207.194 "cd /docker/nova-v2 && docker compose logs agent-35-raster_2d_processor"`
- Check dependencies geïnstalleerd als host tools nodig zijn


## Volgende prompt

`04_fase4_advanced/prompt_28_agent_*.md` (zie PROMPT_INDEX.md)
