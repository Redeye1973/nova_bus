# Prompt 17: Agent 24 - Aseprite Animation Jury

## Wat deze prompt bouwt

Aseprite Animation Jury voor NOVA v2 pipeline.

## Voorwaarden

- [ ] Prompt 16 compleet
- [ ] agent_16_status.json toont "active" of "fallback_mode"

## De prompt

```
Bouw Agent 24 - Aseprite Animation Jury voor NOVA v2.

CONTEXT:
- Spec: L:\!Nova V2\extensions\nova_v2_extensions\aseprite\24_aseprite_animation_jury.md
- Target: L:\!Nova V2\v2_services\agent_24_aseprite_animation_jury\
- Hetzner: /docker/nova-v2/services/agent_24_aseprite_animation_jury/
- Webhook: http://178.104.207.194:5680/webhook/aseprite-anim-review
- Port intern: 8124
...
frame_smoothness, loop_continuity, timing_coherence, silhouette_volume, color_coherence

ENDPOINTS:
/review/rotation, /review/loop, /review/timing

DEPENDENCIES:
pillow numpy opencv-python scikit-image
...
10. Status file schrijven naar L:\!Nova V2\status\agent_24_status.json
...
- Agent 24 Aseprite Animation Jury status
- Webhook URL
- Test resultaten  
...
Invoke-RestMethod -Uri "http://178.104.207.194:5680/webhook/aseprite-anim-review" -Method POST -Body $testBody -ContentType "application/json"

# Check status
Get-Content "L:\!Nova V2\status\agent_24_status.json" | ConvertFrom-Json
```
...
- Check container logs: `ssh root@178.104.207.194 "cd /docker/nova-v2 && docker compose logs agent-24-aseprite_animation_jury"`
- Check dependencies geïnstalleerd als host tools nodig zijn


## Volgende prompt

`03_fase3_polish/prompt_18_agent_*.md` (zie PROMPT_INDEX.md)
