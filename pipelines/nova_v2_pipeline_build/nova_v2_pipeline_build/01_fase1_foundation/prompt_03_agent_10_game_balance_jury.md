# Prompt 03: Agent 10 - Game Balance Jury

## Wat deze prompt bouwt

Game Balance Jury voor NOVA v2 pipeline.

## Voorwaarden

- [ ] Prompt 02 compleet
- [ ] agent_02_status.json toont "active" of "fallback_mode"

## De prompt

```
Bouw Agent 10 - Game Balance Jury voor NOVA v2.

CONTEXT:
- Spec: L:\!Nova V2\agents\nova_v2_agents\jury_judge\10_game_balance_jury.md
- Target: L:\!Nova V2\v2_services\agent_10_game_balance_jury\
- Hetzner: /docker/nova-v2/services/agent_10_game_balance_jury/
- Webhook: http://178.104.207.194:5680/webhook/balance-review
- Port intern: 8110
...
stat_progression_checker, economy_validator, difficulty_curve_analyzer

ENDPOINTS:
/review/stats, /review/economy, /review/difficulty_curve
...
10. Status file schrijven naar L:\!Nova V2\status\agent_10_status.json
...
- Agent 10 Game Balance Jury status
- Webhook URL
- Test resultaten  
...
Invoke-RestMethod -Uri "http://178.104.207.194:5680/webhook/balance-review" -Method POST -Body $testBody -ContentType "application/json"

# Check status
Get-Content "L:\!Nova V2\status\agent_10_status.json" | ConvertFrom-Json
```
...
- Check container logs: `ssh root@178.104.207.194 "cd /docker/nova-v2 && docker compose logs agent-10-game_balance_jury"`
- Check dependencies geïnstalleerd als host tools nodig zijn


## Volgende prompt

`01_fase1_foundation/prompt_04_agent_*.md` (zie PROMPT_INDEX.md)
