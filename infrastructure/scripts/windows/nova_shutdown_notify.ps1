...
try {
    ...
    Add-Content -Path $logPath -Value "[$((Get-Date).ToString('s'))] shutdown notify sent"
} catch {
    Add-Content -Path $logPath -Value "[$((Get-Date).ToString('s'))] shutdown notify failed: $($_.Exception.Message)"
}
exit 0
